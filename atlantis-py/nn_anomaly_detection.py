...
def feature_extracting(dct_idves_to_data):
	...
	for id_ves in list(dct_idves_to_data.keys()):
		...
		if flag_a:
			feature_3 = len(a.id_fish_catched.unique())     # кол-во уникальных пойманных рыб
			feature_2 = len(a.id_region.unique())           # кол-во уникальных регионов ловли
			feature_4 = a.catch_volume.sum()                # суммарное пойманное кол-во рыбы
			feature_6 = a.catch_volume.mean()               # среднее пойманное кол-во рыбы
			feature_9 = len(a.id_own.unique())              # кол-во уникальных владельцев судна
			feature_11 = (max(a.date) - min(a.date)).days   # время всех сессий ловли
			features.extend([feature_3, feature_2, feature_4, feature_6, feature_9, feature_11])
			features_dscr.extend(["вид рыбы", "территория ловли", "тоннаж рыбы", "средний тоннаж рыбы", "владелец судна", "время ловли"])
	        

		if flag_b:
			feature_5 = b.prod_volume.sum()                 # суммарное кол-во рыб для переработк
			feature_7 = b.prod_volume.mean()                # среднее кол-во рыб для перерадотк
			feature_12 = b.prod_board_volume.mean()         # среднее значение итогового объема на борту
			features.extend([feature_5, feature_7, feature_12])
			features_dscr.extend(["суммарное кол-во переработанной рыбы", "среднее кол-во переработанной рыбы", "среднее значение итогового объема на борту"])

		if flag_a and flag_b:
			catched_val = a.catch_volume.sum()              
			prod_val = b.prod_volume.sum()                  
			feature_10 = catched_val - prod_val             # объем пойманное - объем проданной
		...
		if flag_c:
			...
			features.extend([feature_13])                   # средняя разница между временем регистрации и сертификата и временем вылова
...
def get_predict(dct_idves_to_data):
	output_data = {}
	THRESHOLD = 20
	data_a, data_b, data_a_b, data_a_c, data_b_c, data_a_b_c = feature_extracting(dct_idves_to_data)
	...
	for i in range(len(data_a_b[0])):
		...
		probs_dict = {data_a_b[2][i][j]: min(probs[j]*(r[0]*10+1), 100) for j in range(len(probs))}
		r.append(sum(probs_dict.values())/len(probs_dict))
		r.append(probs_dict)
	...
	for i in range(len(data_a_b_c[0])):
		...
		probs_dict = {data_a_b_c[2][i][j]: min(probs[j]*(r[0]*10+1), 100) for j in range(len(probs))}
		r.append(sum(probs_dict.values())/len(probs_dict))
		r.append(probs_dict)
	...
	for i in range(len(data_b[0])):
		...
		probs_dict = {data_b[2][i][j]: round(min(probs[j]*(r[0]*10+1), 100), 3) for j in range(len(probs))}
		r.append(sum(probs_dict.values())/len(probs_dict))
		r.append(probs_dict)
